class Admin::ToolsController < Admin::AdminController
  ...
  def tool_params
    params.require(:tool).permit(:photo, :name, :description, :identifier, :quantity, {photos_attributes: [:id, :picture, :tool_id, :_destroy]}, {documentations_attributes: [:id, :name, :file, :tool_id, :_destroy]}, {labs_tools_attributes: [:id, :lab_id, :tool_id, :quantity, { lab_ids: []}, :_destroy]}, { lab_ids: []}, { category_ids: []})
  end
end
